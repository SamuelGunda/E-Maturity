...
export class EducationMaterialsComponent implements OnInit {
  ...
  constructor(private darkModeService: DarkModeService, private eduService: EduService, private renderer: Renderer2, private el: ElementRef) { }
  ...
  toggleShowSjlNotes() {
    ...
    if (this.showSjlNotes) {
      this.renderer.setStyle(this.el.nativeElement.querySelector('.main_container'), 'height', '119rem');
    }
    else if (this.showAnjNotes) {
      this.renderer.setStyle(this.el.nativeElement.querySelector('.main_container'), 'height', '92rem');
    }
    else {
      this.renderer.setStyle(this.el.nativeElement.querySelector('.main_container'), 'height', '100%');
    }
  }
  toggleShowAnjNotes() {
